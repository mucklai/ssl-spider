...
module.exports.analyze = function (options, callback) {
	...
	// should not start new assessments when max assessments are reached
	if (options.startNew) {
		if (api.info.currentAssessments >= api.info.maxAssessments) {
			err = new Error("Client request rate too high");
			callback(err, null);
			return;
		}
